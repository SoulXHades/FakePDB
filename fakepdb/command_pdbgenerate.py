...
class __fakepdb_pdbgeneration_actionhandler(ida_kernwin.action_handler_t):
    ...
    def activate(self, ctx):
        ...
        # Incase user port the IDA database to another PC resulting in different path as what was registered in IDA
        if not os.path.exists(filepath_exe):
            filepath_exe = os.path.join(idb_dir, pe_filename_ext)
        print('    * exe File path: %s' % filepath_exe)
